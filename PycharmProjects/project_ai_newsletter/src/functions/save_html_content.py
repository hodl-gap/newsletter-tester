"""
Save HTML Content Node

Saves the final HTML-scraped content to JSON and CSV files.
Similar to save_aggregated_content but outputs to html_news.* files.
"""
...
def save_html_content(state: dict) -> dict:
    """
    Save HTML-scraped content to files.

    Args:
        state: Pipeline state with 'output_data' and 'discarded_articles'

    Returns:
        Dict with 'save_status' info
    """
    with track_time("save_html_content"):
        debug_log("[NODE: save_html_content] Entering")
        ...
        debug_log(f"[NODE: save_html_content] Saved JSON to {json_path}")
        ...
        if output_data:
            ...
            debug_log(f"[NODE: save_html_content] Saved CSV to {csv_path}")

        # Save discarded articles CSV
        discarded_csv_path = data_dir / "html_discarded.csv"

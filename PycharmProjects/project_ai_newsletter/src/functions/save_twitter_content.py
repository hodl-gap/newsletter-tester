"""
Save Twitter Content Node

Saves the final Twitter output to JSON and CSV files.
"""
...
def save_twitter_content(state: dict) -> dict:
    """
    Save aggregated Twitter content to files.

    Args:
        state: Pipeline state with 'output_data', 'twitter_accounts', and 'discarded_articles'

    Returns:
        Dict with 'save_status' info
    """
    with track_time("save_twitter_content"):
        debug_log("[NODE: save_twitter_content] Entering")
        ...
        debug_log(f"[NODE: save_twitter_content] Saving {len(output_data)} records")

        # Prepare output paths
        data_dir = Path("data")
        data_dir.mkdir(exist_ok=True)

        json_path = data_dir / "twitter_news.json"
        csv_path = data_dir / "twitter_news.csv"

        # Build metadata
        metadata = {
            "timestamp": datetime.now().isoformat(),
            "source": "twitter",
            "total_tweets": len(output_data),
            "accounts_scraped": [a.get("handle", "") for a in twitter_accounts],
            "cost": _get_cost_info(),
        }
        ...
        for record in output_data:
            ...
            handle = record.get("source", "Unknown")
        ...
        metadata["by_handle"] = handle_counts
        ...
        debug_log(f"[NODE: save_twitter_content] Saved JSON to {json_path}")
        ...
        if output_data:
            ...
            debug_log(f"[NODE: save_twitter_content] Saved CSV to {csv_path}")

        # Save discarded tweets CSV
        discarded_csv_path = data_dir / "twitter_discarded.csv"
...
def _print_summary(metadata: dict, output_data: list, discarded_count: int = 0) -> None:
    ...
    print("\n" + "=" * 60)
    print("TWITTER AGGREGATION COMPLETE")
    print("=" * 60)
    print(f"Total tweets: {metadata['total_tweets']}")
    print(f"Discarded tweets: {discarded_count}")
    print(f"Accounts scraped: {', '.join(metadata.get('accounts_scraped', []))}")
    print()

    print("By Handle:")
    for handle, count in sorted(metadata.get("by_handle", {}).items(), key=lambda x: -x[1]):

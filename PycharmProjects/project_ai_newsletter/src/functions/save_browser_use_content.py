"""
Save Browser-Use Content Node

Saves the final browser-use scraped content to JSON and CSV files.
Similar to save_html_content but outputs to browser_use_news.* files.
"""
...
def save_browser_use_content(state: dict) -> dict:
    """
    Save browser-use scraped content to files.

    Args:
        state: Pipeline state with 'output_data', 'discarded_articles', and 'browser_use_failures'

    Returns:
        Dict with 'save_status' info
    """
    with track_time("save_browser_use_content"):
        debug_log("[NODE: save_browser_use_content] Entering")

        output_data = state.get("output_data", [])
        discarded_articles = state.get("discarded_articles", [])
        failures = state.get("browser_use_failures", [])

        debug_log(f"[NODE: save_browser_use_content] Saving {len(output_data)} records")

        # Prepare output paths (get_data_dir creates the directory if needed)
        data_dir = get_data_dir()

        json_path = data_dir / "browser_use_news.json"
        csv_path = data_dir / "browser_use_news.csv"
        failures_path = data_dir / "browser_use_failures.json"
        ...
        metadata["by_source"] = source_counts
        ...
        debug_log(f"[NODE: save_browser_use_content] Saved JSON to {json_path}")
        ...
        if output_data:
            ...
            debug_log(f"[NODE: save_browser_use_content] Saved CSV to {csv_path}")

        # Save discarded articles CSV
        discarded_csv_path = data_dir / "browser_use_discarded.csv"
        if discarded_articles:
            ...
            debug_log(f"[NODE: save_browser_use_content] Saved {len(discarded_articles)} discarded articles to {discarded_csv_path}")

            # Also store discarded articles in database for debugging/reference
            db = ArticleDatabase()
            db.insert_discarded_batch(discarded_articles, source_type="browser_use")
        ...
        if failures:
            ...
            debug_log(f"[NODE: save_browser_use_content] Saved {len(failures)} failures to {failures_path}")

        # Print summary
        _print_summary(metadata, output_data, len(discarded_articles), len(failures))
...
def _print_summary(metadata: dict, output_data: list, discarded_count: int = 0, failure_count: int = 0) -> None:
    """Print summary to console."""
    print("\n" + "=" * 60)
    print("BROWSER-USE LAYER 2 COMPLETE")
    print("=" * 60)
    print(f"Total articles: {metadata['total_articles']}")
    print(f"Discarded articles: {discarded_count}")
    print(f"Failed sources: {failure_count}")
    print()

    print("By Source:")
    for src, count in sorted(metadata.get("by_source", {}).items(), key=lambda x: -x[1]):
        print(f"  {src}: {count}")
    print()
